...
class DataUtil:
    ...
    def get_dataset(name, path, n_train=None, tar_idx=None, shuffle=True,
        ...
        with open(path, "r", encoding="utf8") as file:
          ...
          #  print(x[0:3]) # [['p', 'x', 's', 'n', 't', 'p', 'f', 'c', 'n', 'k', 'e', 'e', 's', 's', 'w', 'w', 'p', 'w', 'o', 'p', 'k', 's', 'u'], [], ...]
        if shuffle:
            np.random.shuffle(x)
        #print(x[0:3])
        ...
        #print(y[0:3])
        #print(x[0:3])  #[['x' 'f' 'y' 'f' 'f' 'f' 'c' 'b' 'p' 'e' 'b' 'k' 'k' 'b' 'b' 'p' 'w' 'o' 'l' 'h' 'y' 'p']...[]]
    ...
    def quantize_data(x, y, wc=None, continuous_rate=0.1, separate=False):
        # print(x[0:3]) ['x' 'f' 'y' 'f' 'f' 'f' 'c' 'b' 'p' 'e' 'b' 'k' 'k' 'b' 'b' 'p' 'w' 'o'
        #   'l' 'h' 'y' 'p']
        ...
        if isinstance(x, list):
            ...
            xt = x.T  #矩阵转置  把每一类特征分开[[1],[2]...]
        #print(x.T)
        features = [set(feat) for feat in xt] #[[去掉重复的类别]] [{'f', 'c', 'x', 'b', 's', 'k'}, {'f', 'y', 's', 'g'}, {'n', 'u', 'c', 'g', 'b', 'p', 'r', 'e', 'y', 'w'}]
        ...
        #print(wc)
        #print(feat_dicts)
        #   {'f': 0, 'x': 1, 'c': 2, 's': 3, 'k': 4, 'b': 5}, {'f': 0, 's': 1, 'g': 2, 'y': 3}
        if not separate:
            if np.all(~wc):
                dtype = np.int
            else:
