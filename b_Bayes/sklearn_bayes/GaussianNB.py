...
class DataUtil:
    ...
    def get_dataset(name, path, n_train=None, tar_idx=None, shuffle=True,
        ...
        if shuffle:
            np.random.shuffle(x)
        tar_idx = 0 if tar_idx is None else tar_idx
...
def test(xs, ys):
    ...
    plt.plot(train_sizes, nb_scores, color='blue', label="Naive bayes")
    plt.plot(train_sizes, lr_scores, color='red', label='Logistic regression')
    plt.title("naive bayes and logistic regression accuracies")
    plt.xlabel("training instances")
    plt.ylabel("set accuracy")
    plt.show()
...
if __name__=="__main__":
    #train()
    xs, ys = DataUtil.get_dataset("mushroom", "./mushroom.txt", tar_idx=0)
    x, y, _, features, feat_dicts, label_dict = DataUtil.quantize_data(xs, ys, wc=np.array([False] * len(xs[0])))
    y = y.reshape((len(y), 1))
    test(x, y)
