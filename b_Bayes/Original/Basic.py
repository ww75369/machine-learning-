...
class NBfunctions:
    @staticmethod
    def guassaian(x, mu, sigma):
        return exp(-(x - mu) ** 2 / (2 * sigma ** 2)) / (sqrt_pi * sigma)

    @staticmethod
    def guassian_maximum_likelihood(labelled_x, n_category, dim):
        ...
        def func(_c):
            def sub(x):
                return NBfunctions.guassaian(x, mu[_c], sigma[_c])
